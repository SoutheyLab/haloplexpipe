...
def make_pipeline(state):
    ...
    pipeline.transform(
        ...
        filter=formatter('processed_fastqs/(?P<sample>[a-zA-Z0-9_-]+)_R1.fastq.gz(?P<suretrim>[a-zA-Z0-9_-]+).fastq.gz'),
    ...
    pipeline.transform(
        ...
        filter=formatter('processed_fastqs/(?P<sample>[a-zA-Z0-9_-]+).bam'),
        add_inputs=add_inputs(['{path[0]}/{sample[0]}_I2.fastq.gz']),
